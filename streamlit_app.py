...
# Set your own API key # os.environ["GOOGLE_API_KEY"] = 'API KEY HERE'
# Setup model
...
st.title("FEMA Data Explorer")
zip_code = st.text_input("Please enter your zip code:")

if zip_code:
    params = {'reportedZipCode': zip_code}
    ...
    if data:
        ...
        st.success(f"Data has been exported to {csv_file}")
...
tools.extend([total_damage_tool, average_damage_tool, recent_loss_tool, policies_by_zone_tool, total_claims_tool, total_damage_tool])
...
# Example queries
if st.button("What is the total building damage amount?"):
    query_response = agent_executor.invoke({
        "input": "What is the total building damage amount?"
    })
    st.write(query_response["output"])

if st.button("What is the average contents damage amount?"):
    query_response = agent_executor.invoke({
        "input": "What is the average contents damage amount?"
    })
    st.write(query_response["output"])

if st.button("What is the most recent date of loss?"):
    query_response = agent_executor.invoke({
        "input": "What is the most recent date of loss?"
    })
    st.write(query_response["output"])

if st.button("How many policies are there by flood zone?"):
    query_response = agent_executor.invoke({
        "input": "How many policies are there by flood zone?"
    })
    st.write(query_response["output"])

if st.button("What is the total number of claims?"):
    query_response = agent_executor.invoke({
        "input": "What is the total number of claims?"
    })
    st.write(query_response["output"])

if st.button("What is the total sum of building and contents damage amounts?"):
    query_response = agent_executor.invoke({
